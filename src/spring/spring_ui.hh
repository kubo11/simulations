...
class SpringUI : public UI {
 public:
  ...
  std::unique_ptr<Function> get_anchor_position_function();
 ...
 private:
  float m_weight_starting_position = 0.0f;
  float m_weight_starting_velocity = 0.0f;
  float m_dt = 0.01f;
  float m_weight_mass = 1.0f;
  float m_elasticity_coef = 5.0f;
  float m_damping_coef = 0.1f;
  std::vector<std::unique_ptr<Function>> m_anchor_position_functions;
  unsigned int m_selected_anchor_pos_func_idx = 0;
  ...
  std::vector<float> m_anchor_position;
